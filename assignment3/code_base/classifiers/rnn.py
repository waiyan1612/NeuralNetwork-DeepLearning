...
class SentimentAnalysisRNN(object):
    ...
    def loss(self, wordvecs, labels, mask):
        ...
        ############################################################################
        h, h_cache = rnn_forward(wordvecs, h0, Wx, Wh, b)
        ha, ha_cache = temporal_affine_forward(h, W_a, b_a)
        havg, havg_cache = average_forward(ha, mask)
        out = havg.dot(W_fc) + b_fc
        loss, dout = softmax_loss(out, labels)

        grads['W_fc'] = havg.T.dot(dout)
        grads['b_fc'] = dout.sum(axis=0)
        dhavg = average_backward(dout.dot(W_fc.T), havg_cache)
        dha, grads['W_a'], grads['b_a'] = temporal_affine_backward(dhavg, ha_cache)
        _, _, grads['Wx'], grads['Wh'], grads['b'] = rnn_backward(dha, h_cache)
